...
export type AirPhase = 'fall' | 'ground' | 'land' | 'rise'
...
function afterLanding(state: AirState, input: AirInput, timings: AirTimings): AirState {
  ...
  const wasAirborne = state.phase === 'rise' || state.phase === 'fall'
...
export function stepAir(state: AirState, input: AirInput, timings: AirTimings): AirState {
  return advanced(afterLanding(afterTakeoff(state, input, timings), input, timings), input, timings)
}
