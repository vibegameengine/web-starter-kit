...
export const geometries = {
  ...
  // Hexagonal prism (6-sided crystal shard) — flat faces read clearly while it spins.
  prism: new THREE.CylinderGeometry(0.5, 0.5, 1.1, 6),
  // Tapered hex shard — pointed top, wide base: reads unmistakably as a crystal.
  shard: new THREE.CylinderGeometry(0.06, 0.5, 1.5, 6),
...
// Glossy near-white crystal — glassy sheen (very low roughness) plus a faint cool
// inner glow, so it reads as a magical shard rather than white plastic.
export const crystal = new THREE.MeshStandardMaterial({
  color: '#eef4fb',
  roughness: 0.05,
  metalness: 0,
  emissive: '#bcd6f5',
...
export function registerWarmupResources(): void {
  ...
  ShaderWarmupRegistry.register('crystal-shard', geometries.shard, crystal)
}
