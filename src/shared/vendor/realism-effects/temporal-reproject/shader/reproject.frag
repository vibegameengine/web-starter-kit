...
void getVelocityNormalDepthFromTexture(const sampler2D tex, inout vec2 dilatedUv, out vec2 vel, out vec3 normal, out float depth) {
  vec2 centerUv = dilatedUv;

  vec4 velocityTexel = textureLod(tex, centerUv, 0.0);
...
void getVelocityNormalDepth(inout vec2 dilatedUv, out vec2 vel, out vec3 normal, out float depth) {
  getVelocityNormalDepthFromTexture(velocityTexture, dilatedUv, vel, normal, depth);
}

void getLastVelocityNormalDepth(inout vec2 dilatedUv, out vec2 vel, out vec3 normal, out float depth) {
  getVelocityNormalDepthFromTexture(lastVelocityTexture, dilatedUv, vel, normal, depth);
}
...
float validateReprojectedUV(const vec2 reprojectedUv, const vec3 worldPos, const vec3 worldNormal, const bool isHitPoint) {
  ...
  getLastVelocityNormalDepth(dilatedReprojectedUv, lastVelocity, lastWorldNormal, lastDepth);
