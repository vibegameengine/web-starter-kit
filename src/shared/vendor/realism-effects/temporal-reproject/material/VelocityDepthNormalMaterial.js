...
vec4 currentTransformed = vec4( transformed, 1.0 );
#ifdef USE_INSTANCING
currentTransformed = instanceMatrix * currentTransformed;
#endif
newPosition = velocityMatrix * currentTransformed;

// Previous position reuses the CURRENT skinned position, so velocity comes only
// from camera / model movement (prevVelocityMatrix vs velocityMatrix) — not from
// per-bone animation. Gives skinned characters correct, noise-free motion blur.
prevPosition = prevVelocityMatrix * currentTransformed;
