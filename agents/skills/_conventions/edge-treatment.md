# Edge treatment — the chamfer default

A modelling convention shared by every skill in this repository that produces
geometry, kept in one file because it was written once and copied seventeen
times, and a rule that lives in seventeen places is a rule that will be right in
some of them.

**This is a default, not a law.** The numbers below were derived on one game's
world scale — human-height architecture and props seen from a first-person
camera, at metre scale. They are the sizes at which a chamfer reads as a
highlight rather than as a bevel you can see. A project at a different scale, or
with a deliberately flat, faceted or low-poly look, sets its own figures and
says so; what transfers is the reason, which is optical and applies to any
renderer.

Skills that reference this file: `blender-export`, `blender-mcp-hand-modeling`,
`blender-modeling`, `blender-pro-workflow`, `contour-to-mesh`,
`fit-repair-optimizer`, `mascot-logo-reconstruction`, `reference-to-3d`,
`rig-socket-fitting`, `source-part-segmentation`, `text-to-blender`,
`texture-driven-mesh-fitting`, `threejs-authored-vegetation`,
`threejs-procedural-architecture`, `threejs-scene-authoring`, `wireframe-to-3d`.
