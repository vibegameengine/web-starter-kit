# Mobs: animation, ragdoll, and the traps between them

Written after building the imp — the project's first skinned, animated,
collapsing mob — and getting most of it wrong at least once first. Everything
here is a measurement, not an opinion; where a number appears, the command that
produced it is named.

The intended reader is whoever adds the SECOND mob. Most of what follows will
...
```
features/<mob>/
  systems/<mob>Animation.ts    pure — no React, no three, no clock. Testable.
  systems/boneLayers.ts        cuts one take into lower/upper bone layers
  components/use<Mob>Animation.ts   steps the machine on the FIXED tick
  components/useHeadAim.ts     per-frame additive turn on top of the take
  entities/<Mob>/use<Mob>Rig.ts     clips, mixer, crossfades — presentation only
  entities/<Mob>/<Mob>.tsx     the entity, and the API a script drives it with
  ui/<mob>-lab.lab.ts + <Mob>LabScreen.tsx
scenes/<mob>-lab/<Mob>LabScene.tsx
scripts/build-<mob>-model.mjs      the asset pipeline
scripts/verify-<mob>.mjs           the acceptance
```
...
POSE comes back in the optimizer's space — measured on the imp, an inverse-bind
...
they were captured on — knees through the floor, every frame. `build-mob-model.mjs`
matches each take's rig to the character's and bakes the difference in.

### Rest pose and bind pose are two different things

The imp's capture disagreed about them: the skin said the pelvis stands at
...
skeleton of a different size. `build-<mob>-model.mjs` bakes the height.

And CHECK IT AGAINST THE GAME, not against the capture. The imp shipped at the
...
the head capsule has nothing to span. `build-mob-model.mjs` measures it from the
...
| option | what it does | measured on the imp |
|---|---|---|
...
bone axis — measures a GARMENT on any character wearing one. The imp's loincloth
...
uses it today because the mannequin does not opt in. Fix it before the next mob
does.
...
quietly move the ruler. `scripts/verify-imp.mjs` measures the mannequin in
...
about the imp that the control refuted:

- "The imp never settles because its hip capsules overlap." The mannequin
...
those are different fractions on different bodies — the imp's bones reach 60.3%
...
`wip/imp-anim/VERDICTS.md` is a journal of hypotheses: each row carries the
...
## 7. Where a mob is driven from

```ts
const mob = useRef<ImpApi>(null)
<Imp apiRef={mob} ragdoll onDied={...} />

mob.current.drive({ speedMetersPerSecond, facingRadians })  // every frame, no re-render
mob.current.strike('swipe')                                  // upper layer, legs keep running
mob.current.kill({ point, impulse })                         // death and the blow in one call
mob.current.place([x, y, z], yaw)                            // spawn or teleport
mob.current.look(target)                                     // or null to let it go
```
...
bench; they are not about this imp.
...
skin's inverse binds; `Imp` reports it through `read().heightMetres` so nothing
...
fitted to a one-metre imp and still there at 1.70 m, landing near the hips. The
...
The argument was: the imp's corpse spans 102% of what its skeleton spans
standing, the reference mannequin's only 92%, so the imp's ragdoll opens up
...
imp's in a compact combat stance. The ratio then measures how splayed the body
...
the arm bones from both boxes and the gap disappears — imp 1.18-1.24 against
...
better than no reflex. The imp keeps it.
...
The imp carries `buckle: 0`. The mannequin keeps the authored reflex — it is a
...
- **A contact pair on the NECK.** The neck is the one trunk joint without
  ...
  moved the imp 0.612 -> 0.606. Reverted; the reason is written beside the row.
...
For most of this task the imp "settled within four seconds" about three times in
...
`wip/imp-anim/lab.mjs` — 348 lines, seventeen scenes, the single instrument
...
It is `scripts/imp-bench.mjs` now. The captures still go to `wip/`. The test for
